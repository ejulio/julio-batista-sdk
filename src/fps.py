...
class Page:
    ...
    def __init__(self, page: Optional[int]= None, offset: Optional[int] = None, limit: Optional[int] = None):
        if page is not None and (offset is not None or limit is not None):
            raise ValueError("you must set 'page' or ('limit' and 'offset')")
        elif page is None and (offset is None or limit is None):
            raise ValueError("you must set 'page' or ('limit' and 'offset')")
    ...
    def query_string(self) -> str:
        ...
        return f"offset={self._offset}&limit={self._limit}"
